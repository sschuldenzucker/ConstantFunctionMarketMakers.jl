...
```@example quickstart-heropic
using ConstantFunctionMarketMakers
using Plots

# Define a list of different AMMs
amms = [
...
# Initialize the AMMs at TVL $1M and price 1 to receive a list of TradingCurve's
tcs = tc_v_p.(amms, 1e6, 1.0)

# Define the price range of the following plot
plim = (1-60e-4, 1+60e-4)

# For each trading curve, plot its absolute liquidity density curve
pl = plot()
for tc in tcs
...
1. An [`AMM`](@ref) is the abstract configuration of a CFMM. Mathematically, this is just a function $F: \mathbb{R}^2 \to \mathbb{R}$ where the level sets are the trading curves, but computationally, it's not.

2. A [`TradingCurve`](@ref) is a specific trading curve of an AMM, together with an initial point on the curve. Mathematically, this is defined by an invariant $L$ plus a price $p$ and then the curve is equal to $F^{-1}(L)$, but computationally, there is more data.
...
See the [API Reference](api.md) for how to use this library.

To define new AMMs and for other special applications, see the [Internal API](api_internal.md).

# Development

For development see the [readme](https://github.com/sschuldenzucker/ConstantFunctionMarketMakers.jl).
